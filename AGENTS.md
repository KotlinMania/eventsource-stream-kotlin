# Agent guide — eventsource-stream-kotlin

Quick reference for translators. Long-form discipline is in [CLAUDE.md](CLAUDE.md).

## What this repo is

Clean-room Kotlin Multiplatform port of the upstream Rust crate [`eventsource-stream`](https://crates.io/crates/eventsource-stream). Every Kotlin file is a faithful translation of one Rust file. No JVM-only deps, no Java imports, no shortcuts via established Java/Kotlin libraries.

## Workflow per file

1. Clone upstream into `tmp/eventsource-stream/` (gitignored) if not already present.
2. Pick the next file to port — leaves of the dep tree first.
3. Read the whole `.rs` before typing.
4. Create the matching `.kt`. First line: `// port-lint: source <path-relative-to-tmp/eventsource-stream/>`. Second line: `package io.github.kotlinmania.eventsourcestream.<subpkg>`.
...
Provenance gate: `ast_distance --deep tmp/eventsource-stream/src rust src/commonMain/kotlin/io/github/kotlinmania/eventsourcestream kotlin`.
