# Claude Code Project Instructions — eventsource-stream-kotlin

## Project Overview

This is **eventsource-stream-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`eventsource-stream`](https://crates.io/crates/eventsource-stream).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/eventsource-stream/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
